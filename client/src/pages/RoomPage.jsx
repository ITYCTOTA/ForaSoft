...
export default function RoomPage({ roomId, initialName = '' }) {
  ...
  const [participants, dispatchParticipants] = useReducer(participantsReducer, initialParticipantsState); const [messages, dispatchMessages] = useReducer(messagesReducer, initialMessagesState); const sessionRef = useRef(null)
  ...
  const join = () => { const result = validateDisplayName(name); if (!result.ok) return setError(result.message); setError(''); if (!sessionRef.current) sessionRef.current = new RoomSession({ onState: handleState }); sessionRef.current.join({ roomId, displayName: result.value }) }
