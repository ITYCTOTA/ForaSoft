...
export class MediaController {
  ...
  async acquire() {
    ...
    if (!this.secureContext || !this.mediaDevices?.getUserMedia)
      return this.#result(UI_MESSAGES.WEBRTC_UNSUPPORTED);
    ...
    if (requestId !== this.requestId) {
      for (const result of [audio, video])
        if (result.status === "fulfilled") stopTracks(result.value);
      return this.#result();
    }
  ...
  #result(error = null) {
    return {
      stream: this.stream,
      audioEnabled: this.stream.getAudioTracks().some((track) => track.enabled),
      videoEnabled: this.stream.getVideoTracks().some((track) => track.enabled),
      error,
    };
  }
...
function messageFor(results) {
  ...
  if (reasons.includes("NotAllowedError"))
    return UI_MESSAGES.MEDIA_ACCESS_DENIED;
