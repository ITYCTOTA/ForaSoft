...
class FakePeerConnection {
  constructor(config) { this.config = config; this.transceivers = []; this.connectionState = 'new' }
...
describe('PeerConnectionManager', () => {
  it('creates one peer per UUID with STUN and empty media senders', () => {
    const manager = new PeerConnectionManager({ RTCPeerConnectionCtor: FakePeerConnection, MediaStreamCtor: FakeStream })
    const peer = manager.ensurePeer('remote-1')
    expect(manager.ensurePeer('remote-1')).toBe(peer)
    expect(peer.connection.config.iceServers).toEqual([{ urls: 'stun:stun.l.google.com:19302' }])
    expect(peer.connection.transceivers.map((item) => item.kind)).toEqual(['audio', 'video'])
  })
  it('limits peers, emits remote streams and closes only the removed peer', () => {
    const remote = []; const manager = new PeerConnectionManager({ RTCPeerConnectionCtor: FakePeerConnection, MediaStreamCtor: FakeStream, onRemoteStream: (...args) => remote.push(args) })
    const first = manager.ensurePeer('one'); const second = manager.ensurePeer('two'); const third = manager.ensurePeer('three')
    first.connection.emitTrack({ kind: 'video' })
    expect(remote[0][0]).toBe('one'); expect(remote[0][1].tracks).toHaveLength(1)
    expect(() => manager.ensurePeer('four')).toThrow('capacity')
    manager.removePeer('two')
    expect(second.connection.closed).toBe(true); expect(first.connection.closed).toBeUndefined(); expect(third.connection.closed).toBeUndefined()
  })
  it('replaces tracks for existing peers and closes all', async () => {
    const manager = new PeerConnectionManager({ RTCPeerConnectionCtor: FakePeerConnection, MediaStreamCtor: FakeStream }); const peer = manager.ensurePeer('one')
