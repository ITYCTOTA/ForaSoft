...
const mediaStream = class {
  ...
  removeTrack(item) {
    this.tracks = this.tracks.filter((track) => track !== item);
  }
  getTracks() {
    return this.tracks;
  }
...
describe("MediaController", () => {
  ...
  it("keeps available camera when audio permission is denied", async () => {
    ...
    await expect(controller.acquire()).resolves.toMatchObject({
      audioEnabled: false,
      videoEnabled: true,
      error: UI_MESSAGES.MEDIA_ACCESS_DENIED,
    });
  });
  it("returns empty stream without devices and stops tracks on cleanup", async () => {
    const controller = new MediaController({
      MediaStreamCtor: mediaStream,
      mediaDevices: { getUserMedia: () => rejected("NotReadableError") },
    });
    await expect(controller.acquire()).resolves.toMatchObject({
      audioEnabled: false,
      videoEnabled: false,
    ...
    const audio = track("audio");
    controller.stream.addTrack(audio);
    controller.stop();
  ...
  it("does not retain late tracks after cancellation", async () => {
    ...
    const controller = new MediaController({
      ...
      mediaDevices: {
        getUserMedia: ({ audio: wantsAudio }) =>
          wantsAudio ? late : Promise.resolve(stream()),
      },
    });
    const pending = controller.acquire();
    controller.stop();
    const audio = track("audio");
    resolveAudio(stream(audio));
    await pending;
    expect(audio.stopped).toBe(true);
  });
  it("toggles an available microphone without recreating media", async () => {
    ...
    const controller = new MediaController({
      ...
      mediaDevices: {
        getUserMedia: ({ audio: wantsAudio }) =>
          Promise.resolve(wantsAudio ? stream(audio) : stream()),
      },
    });
  ...
  it("stops camera and can acquire a new video track", async () => {
    ...
    const controller = new MediaController({
      ...
      mediaDevices: {
        getUserMedia: ({ video }) =>
          Promise.resolve(
            video ? stream(calls++ === 0 ? first : second) : stream(),
          ),
      },
    });
    await controller.acquire();
    expect(controller.disableVideo()).toMatchObject({ videoEnabled: false });
    expect(first.stopped).toBe(true);
    await expect(controller.enableVideo()).resolves.toMatchObject({
      videoEnabled: true,
    });
    expect(controller.stream.getVideoTracks()[0]).toBe(second);
  });
  it("keeps the other media kind when a device track ends", async () => {
    ...
    const controller = new MediaController({
      ...
      mediaDevices: {
        getUserMedia: ({ audio: wantsAudio }) =>
          Promise.resolve(wantsAudio ? stream(audio) : stream(video)),
      },
    });
    await controller.acquire();
    audio.onended();
