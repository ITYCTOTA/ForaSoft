...
describe("RoomRegistry", () => {
  ...
  it("keeps duplicate display names as separate participants", () => {
    ...
    expect(
      [...registry.get("room").participants.values()].map(
  ...
  it("enforces four participants and rejects the fifth without changing state", () => {
    ...
    expect(registry.get("room").participants.size).toBe(
  ...
  it("rejects an invalid generated identity instead of creating an unsafe participant", () => {
    ...
    expect(() =>
      registry.join({ roomId: "room", displayName: "Алекс" }),
    ).toThrow("UUID v4");
  ...
  it("removes a participant, records a leave event and deletes empty rooms", () => {
    ...
    expect(left.participant).toEqual({
  ...
  it("updates only the participant media state", () => {
    ...
    const joined = registry.join({ roomId: "room", displayName: "Анна" });
    expect(
      registry.updateMediaState("room", joined.participant.id, {
        audioEnabled: true,
        videoEnabled: true,
      }),
    ).toEqual({
    ...
    expect(
      registry.get("room").participants.get(joined.participant.id).videoEnabled,
    ).toBe(true);
    expect(
      registry.updateMediaState("room", "missing", {
        audioEnabled: true,
        videoEnabled: true,
      }),
    ).toBeNull();
