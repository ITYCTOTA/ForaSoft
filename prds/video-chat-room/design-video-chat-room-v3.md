...
| --- | --- | --- |
| Unit: server | `RoomRegistry`, validation, rate limiter | Atomic 3→4→full, cleanup at zero, duplicate names, unknown ID creates room, duplicate leave, message cap; новый UUID при каждом входе, согласованность ключа Map и Participant.id, очистка транспортных привязок, обязательные поля обоих вариантов сообщений, сохранение снимков автора/участника после выхода. |
...
| Integration: Socket.io | Real server + multiple socket clients | Join snapshot, signaling isolation between rooms, message history, system events, invalid target, disconnect frees slot, reconnection disabled client config; адресация по participant UUID, запрет подмены автора и отправки системных сообщений клиентом. |
...
Домен предоставит Render.com, TLS certificate предоставит Render, будет использоваться reverse proxy на стороне рендера.
...
Используем встроенные логи и метрики без внешних сервисов. Хранить логи будем на протяжении 7ми дней.
...
Нужны фразы: «Нет доступа к камере или микрофону. Вы можете продолжить без них и изменить разрешение в настройках браузера.», «Ваш браузер не поддерживает видеозвонки WebRTC. Используйте актуальную версию Chrome, Firefox или Edge.», «Не удалось установить медиасоединение с этим участником.», «Соединение с сервером потеряно. Войдите снова.», «Комната заполнена.»

До ответа на TBD реализация может начинаться с локального HTTPS/localhost режима, одного server instance и указанного in-memory ограничения. Изменение этих допущений должно привести к следующей версии `design-video-chat-room-v4.md`.
